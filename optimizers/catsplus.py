    ...
    with open(out_csv, "w", newline="", encoding="utf-8") as f:
        ...
        def log_row(eval_id, score, cfg, out):
            ...
            writer.writerow([
                ...
                float(score),
                json.dumps(cfg),
                json.dumps({} if light_log else {k: (float(v[0]) if isinstance(v, list) else float(v)) if hasattr(v, '__iter__') or isinstance(v, (float, int, getattr(__import__('numpy'), 'floating', float))) else v for k, v in out.items()}),
            ])
        ...
        for eval_id in range(n_warmup, n_evals):
            ...
            if (eval_id + 1) % 25 == 0 or eval_id == n_evals - 1:
                f.flush()
                print(f"[catsplus {eval_id+1}/{n_evals}] score={score:.6f} best={best_score:.6f} elites={elite_n}")
